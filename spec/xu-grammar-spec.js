const expectLegalHashComment = tokens => {
    ...
    return expect(tokens[1].scopes).toEqual(["source.xu", "comment.line.number-sign.xu"]);
};
...
const expectLegalSlashComment = tokens => {
    ...
    return expect(tokens[1].scopes).toEqual(["source.xu", "comment.line.double-slash.xu"]);
};

describe("Xù grammar", () => {
    let grammar = null;
    beforeEach(() => {
        waitsForPromise(() => atom.packages.activatePackage("language-mscgen"));
        return runs(() => grammar = atom.grammars.grammarForScopeName("source.xu"));
    });
    it("parses the grammar", () => {
        expect(grammar).toBeTruthy();
        return expect(grammar.scopeName).toBe("source.xu");
    });
    describe("A simple, complete (but invalid) Xù script", () => {
        ...
        describe("outside attribute blocks", () => {
            ...
            return it("classifies xu specific arc types as arc type", () => expect(lines[16][3]).toEqual({
                value: "alt",
                scopes: ["source.xu", "storage.type.xu"]
            }));
        });
        return describe("within attribute blocks", () => {
            ...
            describe("within strings", () => {
                ...
                return it("recognizes escaped characters", () => expect(lines[14][12]).toEqual({
                    value: "\\n",
                    scopes: [
                        "source.xu",
                        "keyword.operator.xu",
    ...
    return describe("Outside msc {} scope", () => {
        ...
        return it("declares stuff illegal that would be legal within msc {} scope", () => {
